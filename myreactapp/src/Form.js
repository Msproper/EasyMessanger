...
const App = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.firstName || formData.firstName.trim().length < 3) {
      newErrors.firstName =
        'Имя слишком короткое или содержит недопустимые символы.'
    }
    if (!formData.lastName || formData.lastName.trim().length < 3) {
      newErrors.lastName =
        'Фамилия слишком короткая или содержит недопустимые символы.'
    }
    ...
    if (!formData.password || formData.password.length < 8) {
      newErrors.password =
        'Пароль слишком короткий. Используйте заглавные буквы, цыфры, специальные символы'
    }
